...
这种方式就是先将一个字符串用哈希表计数，再从另一个字符串中遍历不停从哈希表中减去字符的计数，如果哈希表中不含有某一字符可以直接返回 false，或者判断最后哈希表中是否每个 key 对应的值都为 0

时间复杂度超过 44.22% 提交，空间复杂度超过 45.88% 提交
...
var isAnagram = function(s, t) {
  ...
  for (let j of t) {
    ...
    else map.set(j, map.get(j) - 1);
  }
  return Array.from(map).map(i => i[1]).every(j => j === 0)
};
```

第二种解法：排序法

这种思路就是通过 `charCodeAt()` 方法将每个字符串的字符转为 Unicode 码点，然后排序后再转回为全部由数字组成的字符串，比较两个字符串是否完全一样就可以

时间复杂度超过 26.37% 提交，空间复杂度超过 5.07% 提交

```js
var isAnagram = function(s, t) {
  const arr1 = [], arr2 = []
  for (let i of s) {
    arr1.push(i.charCodeAt())
  }
  arr1.sort((a, b) => a - b)
  for (let j of t) {
    arr2.push(j.charCodeAt())
  }
  arr2.sort((a, b) => a - b)
...
时间复杂度超过 17.74% 提交，空间复杂度超过 51.49% 提交
...
var firstUniqChar = function(s) {
  ...
  const arr = Array.from(map).filter(i => i[1] === 1)
  if (arr.length === 0) return -1;
  return s.indexOf(arr[0][0])
};
```

第二种解法：API 法

这种就更简单了，直接用 `indexOf` 和 `lastIndexOf` 两个 API 就可以了，和本组 #2124 的第二种解法思路一样

时间复杂度超过 47.65% 提交，空间复杂度超过 74.80% 提交
...
这种思路就是先通过遍历原字符串统计缺勤和迟到的索引数组，然后再对迟到的进行一下遍历就可以了

时间复杂度超过 87.19% 提交，空间复杂度超过 13.79% 提交
...
var checkRecord = function(s) {
  ...
  for (let i = 0; i < s.length; i++) {
    if (s[i] === 'A') a.push(i);
    else if (s[i] === 'L') l.push(i)
  }
  let lateFlag = false;
  for (let i = 0; i < l.length - 2; i++) {
    if (l[i + 1] - l[i] === 1 && l[i + 2] - l[i + 1] === 1) lateFlag = true;
  }
  if (a.length < 2 && lateFlag === false) return true;
  return false
};
```
...
这种思路就是特别想用正则先处理一下字符串中的字母以外的字母，然后剩下的就是常规思路了

时间复杂度超过 40.31% 提交，空间复杂度超过 41.08% 提交

```js
var mostCommonWord = function(paragraph, banned) {
  const temp = paragraph.match(/\w+/g), arr = [], map = new Map()
  for (let i of temp) {
  ...
  for (let i of freq) {
    if (banned.indexOf(i[0]) === -1) return i[0]
  }
};
```
...
这种是纯纯低端笨比解法，而且我一开始甚至写的更笨比，虽然思路是对的，但提交会超时，不怕笑话也先放出来

```js
// 超出时间限制版本
var removeDuplicates = function(s) {
  if (s.length === 1) return s;
  const scanDuplicates = (s) => {
    for (let i = 1; i < s.length; i++) {
      if (s[i] === s[i - 1]) return false;
    }
    return true
  }
  while (!scanDuplicates(s)) {
    for (let i = 1; i < s.length; i++) {
      if (s[i] === s[i - 1]) s = s.substring(0, i - 1) + s.substring(i + 1)
    }
...
然后才想到可以不用这么笨比稍微转一点弯在一个 for 循环里重新给 `i` 赋值就能好一些，但时间复杂度肯定很难看

时间复杂度超过 5.12% 提交，空间复杂度超过 95.08% 提交
...
这种思路和这里所有其他哈希表法一样思路都非常清晰，只不过这里我给写的稍微多一点点判断和处理，也就是根据题目不同细节需要有不同注意的地方，别的就没啥了

时间复杂度超过 86.79% 提交，空间复杂度超过 16.04% 提交
...
var maxNumberOfBalloons = function(text) {
  ...
  map.forEach((value, key) => {
    if (key === 'l' || key === 'o') map.set(key, Math.floor(value / 2))
  })
  const arr = Array.from(map).filter(i => i[0] === 'b' || i[0] === 'a' || i[0] === 'l' || i[0] === 'o' || i[0] === 'n').map(j => j[1])
  if (arr.some(i => i === 0) || arr.length < 5) return 0;
...
第一种解法：startsWith API 法

这种思路是自己想的，就用遍历加两个 API `startsWith()` 和 `substring()` 硬解就够了，正好可以复习一下这两个 API 以免像某人一样把 `substring` 完全用反在那死循环呢还，是谁我不说

时间复杂度超过 85.81% 提交，空间复杂度超过 77.42% 提交
...
这种思路就是最暴力的用 `charCodeAt()` 和 `fromCharCode()` 就可以了，非常简单

时间复杂度超过 73.08% 提交，空间复杂度超过 43.59% 提交
...
var replaceDigits = function(s) {
  ...
  for (let i = 1; i < temp.length; i++) {
    if (i % 2 !== 0) {
      temp[i] = String.fromCharCode(temp[i - 1].charCodeAt() + Number(temp[i]))
    }
  }
  return temp.join('')
};
...
思路非常简单，也是我现在愿意优先想的解法，就是暴力咔咔无脑写，最后判断条件也不复杂

时间复杂度超过 54.79% 提交，空间复杂度超过 10.96% 提交
...
var kthDistinct = function(arr, k) {
  ...
  const temp = Array.from(map).filter(i => i[1] === 1).map(j => j[0])
  if (temp.length && temp.length >= k) return temp[k - 1];
  else return ''
};
```
...
这种就是最笨最笨的解法，核心思路就是遍历加转数组配合转大写小写 API，讲究的就是一个粗暴无脑就完事了，也不怕各位笑话，五一前最后一个工作日的下午只秉承着能做出来就得的原则，是真坐不住啊

时间复杂度超过 60.87% 提交，空间复杂度超过 26.09% 提交

```js
var capitalizeTitle = function(title) {
  const arr = title.split(' '), res = []
  for (let i of arr) {
    if (i.length <= 2) res.push(i.toLowerCase());
    else {
      const temp = i.split('')
      for (let j = 0; j < temp.length; j++) {
        if (j === 0) temp[j] = temp[j].toUpperCase()
        else temp[j] = temp[j].toLowerCase()
      }
      res.push(temp.join(''))
    }
  }
...
这种思路一句话就是用 ASCII 码来判断更靠后的字母，别的都没啥

时间复杂度超过 28.30% 提交，空间复杂度超过 51.89% 提交

```js
var greatestLetter = function(s) {
  let best = ''
  for (let i of s) {
    if (i.charCodeAt() >= 65 && i.charCodeAt() <= 90 && s.indexOf(i.toLowerCase()) !== -1) {
      if (best === '') best = i;
      else best = String.fromCharCode(Math.max(i.charCodeAt(), best.charCodeAt()))
    }
  }
  return best
};
...
这种思路跟上面的唯一区别就是用正则 `match` API 代替 `charCodeAt` API

时间复杂度超过 39.62% 提交，空间复杂度超过 5.66% 提交

```js
var greatestLetter = function(s) {
  let best = '', regex = /[A-Z]/g
  for (let i of s) {
    if (i.match(regex) && s.indexOf(i.toLowerCase()) !== -1) {
      if (best === '') best = i;
      else best = String.fromCharCode(Math.max(i.charCodeAt(), best.charCodeAt()))
    }
  }
...
这道题我都已经不知道路过多少次而不做了。。。真的很多次看通过率这么高然后进来两分钟没看懂题再关掉，终于咬咬牙甜蜜的做了心理准备非要把这道题看懂。。。

其实无脑解就是求偶数位的竖线和奇数位的竖线之间的星号的和，所以就无脑拆数组就完事了

时间复杂度超过 54.55% 提交，空间复杂度超过 9.09% 提交
...
var countAsterisks = function(s) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (i % 2 === 0) {
      count += arr[i].split('').filter(j => j === '*').length;
    }
  }
  return count
};
...
这种思路要感谢[这位大神](https://leetcode.cn/problems/count-asterisks/solution/by-joey91-utxt/)的细致讲解，正则对我这种 RZ 来说不讲真的光看是看不懂的。。。。

第一部分 `\|.*?\|` 表示 `|内容|` ，其中 `\|` 用来匹配 `|` 字符，`.*?` 用来匹配任意字符

第二部分 `[^*]` 表示非 `*` 的字符，将第一部分都替换为空字符串后再通过 `|` 或 来连接第二部分将 `|内容|` 以外的非 `*` 字符也都替换为空字符串就可以了

时间复杂度超过 90.91% 提交，空间复杂度超过 91.82% 提交
...
直接判断最末尾的是不是 0，通过循环不停 `pop()` 就可以了

时间复杂度超过 65.22% 提交，空间复杂度超过 33.54% 提交
...
时间复杂度超过 81.99% 提交，空间复杂度超过 25.47% 提交
...
这种就突破限制用了额外的数据结构，但是思路太简单了

时间复杂度超过 15.09% 提交，空间复杂度超过 87.39% 提交

```js
var isUnique = function(astr) {
  const set = [...new Set(astr.split(''))]
...
这种是不突破限制的思路，嵌套循环逐个字符串比较 Unicode 码异或是否为 0，如有为 0 则说明有重复字符

时间复杂度超过 58.56% 提交，空间复杂度超过 97.30% 提交

```js
var isUnique = function(astr) {
  for (let i = 0; i < astr.length - 1; i++) {
    for (let j = i + 1; j < astr.length; j++) {
      if ((astr[i].charCodeAt() ^ astr[j].charCodeAt()) === 0) return false;
    }
  }
  return true
};
...
这种就直接上字符串 `substring` + `replaceAll` API 就可以了，完全没技巧

时间复杂度超过 84.28% 提交，空间复杂度超过 59.12% 提交
...
这道题感觉读不懂。。。看了评论才知道题意是要干什么，即【每个字符的数量只要是偶数就能组成回文，奇数的字符最多只允许有一个】，因此直接无脑哈希表，但这里另外注意如果哈希表的 size 为 1 的话说明也是回文

时间复杂度超过 65.04% 提交，空间复杂度超过 69.11% 提交
...
var canPermutePalindrome = function(s) {
  ...
  const arr = Array.from(map).map(i => i[1])
  return arr.filter(i => i % 2 === 1).length <= 1 || map.size === 1
};
```

第二种解法：Set 法

这种思路就是转为 Set 结构后判断长度就可以了，一共有四种情况，分别对应 `aabbcc` / `aabbc` / `aabbccc` / `aaa` 

时间复杂度超过 95.94% 提交，空间复杂度超过 56.10% 提交

```js
var canPermutePalindrome = function(s) {
  const len = [... new Set(s)].length
  return len === s.length / 2 || len === (s.length + 1) / 2 || len === (s.length - 1) / 2 || len === 1
};
```
