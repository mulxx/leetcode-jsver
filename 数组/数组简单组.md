...
这种思路就是就是按照题意不停循环排序并缩短数组直至数组的长度为 1 或 0，完全没有 out-of-box 的想法

时间复杂度超过 61.61% 提交，空间复杂度超过 58.00% 提交
...
var lastStoneWeight = function(stones) {
  while (stones.length > 1) {
    ...
    let b = stones.shift()
    if (a === b) continue;
    else if (a > b) stones.push(a - b);
    else stones.push(b - a)
  }
...
这种骚解法是我拍脑袋想到的，先创建一个 `target` 数组的 Map 结构，然后遍历 `arr`，如果 Map 中包含其中元素则将计数减 1，最后判断 Map 结构是否所有值都为 0 就 OK

时间复杂度超过 96.00% 提交，空间复杂度超过 40.00% 提交
...
var canBeEqual = function(target, arr) {
  ...
  const temp = Array.from(map).map(i => i[1]);
  const res = [...new Set(temp)]
...
这种就是将两个数组都排序后同时遍历，如果不相等就直接返回 false，因为题意翻译一下其实就是两个数组是否含有完全相同的元素

时间复杂度超过 77.33% 提交，空间复杂度超过 34.67% 提交
...
这种思路就完全按照题意嵌套 for 循环来解，只要排除掉最后一个元素直接最后加入结果数组就可以了

时间复杂度超过 85.71% 提交，空间复杂度超过 48.21% 提交
...
这种思路就是把每个年份都放进哈希表里，再稍微辗转腾挪一下就行了，思路不难，感觉无论作为面试题考一下 API 的扎实程度和实际工作业务中二维数组这种都能用得上，我个人还挺喜欢这道题的捏

时间复杂度超过 75.76% 提交，空间复杂度超过 6.06% 提交
...
var maximumPopulation = function(logs) {
  ...
  const max = Math.max(...arr.map(i => i[1]))
  return Math.min(...arr.filter(i => i[1] === max).map(i => i[0]))
};
```
...
时间复杂度超过 58.97% 提交，空间复杂度超过 87.18% 提交
...
这种我确实是想不出来，这里是官方题解的思路，确实非常巧妙，而且也有点抽象，核心是把减法看成加法，即 `nums[i] = nums[j] + k` 或者 `nums[i] = nums[j] - k`

时间复杂度超过 91.03% 提交，空间复杂度超过 5.13% 提交

```js
var countKDifference = function(nums, k) {
  let count = 0, const map = new Map()
  for (let i = 0; i < nums.length; i++) {
    if (map.get(nums[i] + k)) count += map.get(nums[i] + k);
...
这种就是盯着示例 2 试图找到规律，然后从大到小排序，`[9,7,6,5,2,2]` -> `[9,7,5,2]`，也就是跳过 3 的倍数就可以

时间复杂度超过 84.78% 提交，空间复杂度超过 65.22% 提交
...
这道题确实原题根本看不懂要干嘛，还是看了评论里的老哥说 「就是计算出数组中除最大值和最小值的元素个数」才知道是啥意思。。。

第一种解法：filter API 法一

时间复杂度超过 71.43% 提交，空间复杂度超过 72.62% 提交

```js
var countElements = function(nums) {
  const max = Math.max(...nums), min = Math.min(...nums)
  return nums.filter(i => i < max && i > min).length;
};
```

第二种解法：filter API 法二

相比起上面的思路，这种需要额外考虑数组中只有一个数字的情况，即 `max === min`，有点添蛇画足了

时间复杂度超过 71.43% 提交，空间复杂度超过 91.67% 提交

```js
var countElements = function(nums) {
  const max = Math.max(...nums), min = Math.min(...nums)
  const maxLen = nums.filter(i => i === max).length
  const minLen = nums.filter(i => i === min).length
  if (max === min) return nums.length - maxLen;
  else return nums.length - maxLen - minLen
};
```

第三种解法：排序法

时间复杂度超过 15.48% 提交，空间复杂度超过 72.62% 提交

```js
var countElements = function(nums) {
  nums.sort((a, b) => a - b)
  return nums.filter(i => i !== nums[0] && i !== nums[nums.length - 1]).length
};
```
...
这个思路就是每次增加计数时都对数组重新排序，如果有两个以上大于 0，就对前两个分别减 1，反之说明数组为 [1,0,0] 代表还剩最后一杯，这种就无视冷热的区别全部打乱，只看数组中的数字，由于每次都要排序，所以效率肯定是不高

时间复杂度超过 46.94% 提交，空间复杂度超过 6.12% 提交

```js
var fillCups = function(amount) {
  let count = 0
  while (amount.some(i => i > 0)) {
    amount.sort((a, b) => b - a)
    if (amount.filter(j => j > 0).length >= 2) {
      amount[0] -= 1
      amount[1] -= 1
    } else {
      amount[0] -= 1
...
就是先找到最大值和最小值后通过 `splice` API 将它俩从数组中移除出去，然后随便 `pop` 一个出来就可以了

时间复杂度超过 87.25% 提交，空间复杂度超过 53.92% 提交
...
这种就直接遍历出一个既不是最大值也不是最小值的就为答案

时间复杂度超过 47.55% 提交，空间复杂度超过 98.53% 提交
...
这道题在正确解法之前我先展示一下我这个经典还有一个测试用例过不去的思路，没错，就是那个 `[11, 0, 9, 0, 0]` 的用例，我也同样没有认真读题以为只有一副牌所以最多只能有两个王。。。。。，为此我的解法是再看完全脑残的解法。。。。
...
var isStraight = function(nums) {
  ...
  if (nums[0] === 0 && nums[1] === 0) {
    if (setLen !== 4) return false;
    else return nums[4] - nums[2] <= 4
  } else if (nums[0] === 0 && nums[1] !== 0) {
    if (setLen !== 5) return false;
    else return nums[4] - nums[1] === 3 || nums[4] - nums[1] === 4          
  } else {
    if (setLen !== 5) return false;
    else return nums[4] - nums[0] === 4
  }
};
```
...
无奈上面的只能推倒重来，想了一下还是遍历为基础，然后还是把数组分为 0 和非 0 的两部分分开处理，对非 0 的部分拍下序然后遍历，在遍历之前再检查一下是否有重复的数字

遍历时的判断就用 0 的数量作为起始值，每当有不连续的相邻两个数字就累计的减去差，最后判断 0 的数量是否还大于等于 0 就 OK 了，时间复杂度也还算蛮好看

时间复杂度超过 95.52% 提交，空间复杂度超过 76.76% 提交

```js
var isStraight = function(nums) {
  const temp = nums.filter(i => i !== 0)
  if (new Set(temp).size !== temp.length) return false;
  let zeroCount = nums.length - temp.length
  temp.sort((a, b) => a - b)
  for (let i = 0; i < temp.length - 1; i++) {
    if (temp[i + 1] - temp[i] !== 1) zeroCount -= (temp[i + 1] - temp[i] - 1) 
  }
  return zeroCount >= 0
};
```
